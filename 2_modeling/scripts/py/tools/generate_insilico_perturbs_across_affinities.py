...
is_mnase = 'no'
    ...
    for t in tqdm(range(null_seqs.shape[0])):
        ...
        seqsA = [insert_motif(seq = null_seq, motif = sA, position = motifA_input_position) for sA in motifA_seqs_df.seq.values]
        seqsA_n = len(seqsA)
        injected_seqs = injected_seqs + seqsA
        left_df = pd.DataFrame([motifA_seqs_df.seq.values, [motifA_name]*seqsA_n, ['none']*seqsA_n, [motifB_name]*seqsA_n, ['A']*seqsA_n]).transpose()
        left_is_motifA_df = pd.concat([left_is_motifA_df, left_df])
        right_df = pd.DataFrame([['none']*seqsA_n, [motifB_name]*seqsA_n, motifA_seqs_df.seq.values, [motifA_name]*seqsA_n, ['B']*seqsA_n]).transpose()
        right_is_motifA_df = pd.concat([right_is_motifA_df, right_df])

        if motifB_name != 'empty':
            seqsB = [insert_motif(seq = null_seq, motif = sB, position = motifB_input_position) for sB in motifB_seqs_df.seq.values]
            seqsB_n = len(seqsB)
            injected_seqs = injected_seqs + seqsB

            left_df = pd.DataFrame([['none']*seqsB_n, [motifA_name]*seqsB_n, motifB_seqs_df.seq.values, [motifB_name]*seqsB_n, ['B']*seqsB_n]).transpose()
            left_is_motifA_df = pd.concat([left_is_motifA_df, left_df])
            right_df = pd.DataFrame([motifB_seqs_df.seq.values, [motifB_name]*seqsB_n, ['none']*seqsB_n, [motifA_name]*seqsB_n, ['A']*seqsB_n]).transpose()
            right_is_motifA_df = pd.concat([right_is_motifA_df, right_df])

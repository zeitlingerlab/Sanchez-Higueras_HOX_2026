...
check_chromosome_boundaries<-function(gr, resize_boundary, genome=NULL){
  ...
  gr_merged<-merge(as.data.frame(gr), key, by.x="seqnames", by.y="seqnames") %>% makeGRangesFromDataFrame(keep.extra.columns = T, seqnames.field = 'seqnames', start.field = 'start', end.field = 'end', strand.field = 'strand')

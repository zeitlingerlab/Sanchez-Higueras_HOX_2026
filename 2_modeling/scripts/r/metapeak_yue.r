...
get_multi_gr_bw_exo_metapeak <- function(gr.list = gr.list, bw.list = bw.list, upstream = 500, downstream = 500, smooth = NA){
  mclapply(names(gr.list), function(x){
    gr <- gr.list[[x]]
    get_multi_bw_exo_metapeak(gr,bw.list, upstream = upstream, downstream = downstream, smooth = smooth) %>%mutate(grange_name = x)# The sample_name will be the bw name
  },mc.cores = 10) %>% bind_rows() %>% rename(bw_name = sample_name)
}
